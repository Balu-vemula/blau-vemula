...
context('Verifying the functionality of Damages Cost Per Car KPI', () => {
	describe('Verifying the functionality of Damages Cost Per Car KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title*="Damages Cost Per Car"]',0)
				robotEyes.seesTitleOfKPI('[title*="Damages Cost Per Car"]','Damages Cost Per Car (YTD)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title*="Damages Cost Per Car"]','Damages Cost Per Car (YTD)')
			})

			it('Verifying the graph and the value of the Damages Cost Per Car on the tile view', () => {
				robotHands.scrollingIntoTile('[title*="Damages Cost Per Car"]',0)
				robotHands.clickOnAccountingIcon()
				robotHands.verifiyingTheTileWithDefaultFrequency(Cypress.env('DamagesCostPerCar'),"damages_cost_per_car")
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="Damages Cost Per Car "]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="Damages Cost Per Car "]','Damages Cost Per Car ')
			})

			it('Verifying the Damages Cost Per Car drill-down view', () => {
				robotHands.clickOnTile('div[title="Damages Cost Per Car "]')
			...
			it('Verifying the context of Table', () => {
				...
				robotHands.verifyingQuarterlyFrequency(Cypress.env('DamagesCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesYearlyFilterOption()
				robotHands.clickOnYearlyFilterOption()
				robotHands.verifyingYearlyFrequency(Cypress.env('DamagesCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesMonthlyFilterOption()
				robotHands.clickOnMonthlyFilterOption()
				robotHands.verifyingMonthlyFrequency(Cypress.env('DamagesCostPerCar'))
