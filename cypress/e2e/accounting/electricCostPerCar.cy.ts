...
context('Verifying the functionality of Electric Cost Per Car KPI', () => {
	describe('Verifying the functionality of Electric Cost Per Car KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title*="Electric Cost Per Car"]',0)
				robotEyes.seesTitleOfKPI('[title*="Electric Cost Per Car"]','Electric Cost Per Car (YTD)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title*="Electric Cost Per Car"]','Electric Cost Per Car (YTD)')
			})

			it('Verifying the graph and the value of the Electric Cost Per Car on the tile view', () => {
				robotHands.scrollingIntoTile('[title*="Electric Cost Per Car"]',0)
				robotHands.clickOnAccountingIcon()
				robotHands.verifiyingTheTileWithDefaultFrequency(Cypress.env('ElectricCostPerCar'),"electric_cost_per_car")
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="Electric Cost Per Car "]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="Electric Cost Per Car "]','Electric Cost Per Car ')
			})

			it('Verifying the Electric Cost Per Car drill-down view', () => {
				robotHands.clickOnTile('div[title="Electric Cost Per Car "]')
		 ...
		 RoleFilter(['regression'], () => {
			it('Verifying the context of Table', () => {
				...
				robotHands.verifyingQuarterlyFrequency(Cypress.env('ElectricCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesYearlyFilterOption()
				robotHands.clickOnYearlyFilterOption()
				robotHands.verifyingYearlyFrequency(Cypress.env('ElectricCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesMonthlyFilterOption()
				robotHands.clickOnMonthlyFilterOption()
				robotHands.verifyingMonthlyFrequency(Cypress.env('ElectricCostPerCar'))
