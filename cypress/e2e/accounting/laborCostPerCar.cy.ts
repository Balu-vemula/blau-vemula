...
context('Verifying the functionality of Labor Cost Per Car KPI', () => {
	describe('Verifying the functionality of Labor Cost Per Car KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title*="Labor Cost Per Car"]',0)
				robotEyes.seesTitleOfKPI('[title*="Labor Cost Per Car"]','Labor Cost Per Car (YTD)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title*="Labor Cost Per Car"]','Labor Cost Per Car (YTD)')
			})

			it('Verifying the graph and the value of the Labor Cost Per Car on the tile view', () => {
				robotHands.scrollingIntoTile('[title*="Labor Cost Per Car"]',0)
				robotHands.clickOnAccountingIcon()
				robotHands.verifiyingTheTileWithDefaultFrequency(Cypress.env('LaborCostPerCar'),"labor_cost_per_car")
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="Labor Cost Per Car "]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="Labor Cost Per Car "]','Labor Cost Per Car ')
			})

			it('Verifying the Cost Per Car drill-down view', () => {
				robotHands.clickOnTile('div[title="Labor Cost Per Car "]')
		 ...
		 RoleFilter(['regression'], () => {
			it('Verifying the context of Table', () => {
				...
				robotHands.verifyingQuarterlyFrequency(Cypress.env('LaborCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesYearlyFilterOption()
				robotHands.clickOnYearlyFilterOption()
				robotHands.verifyingYearlyFrequency(Cypress.env('LaborCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesMonthlyFilterOption()
				robotHands.clickOnMonthlyFilterOption()
				robotHands.verifyingMonthlyFrequency(Cypress.env('LaborCostPerCar'))
