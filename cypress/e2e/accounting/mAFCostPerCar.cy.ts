...
context('Verifying the functionality of MAF Cost Per Car KPI', () => {
	describe('Verifying the functionality of MAF Cost Per Car KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title*="MAF Per Car"]',0)
				robotEyes.seesTitleOfKPI('[title*="MAF Per Car"]','MAF Per Car (YTD)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title*="MAF Per Car"]','MAF Per Car (YTD)')
			})

			it('Verifying the graph and the value of the MAF Cost Per Car on the tile view', () => {
				robotHands.scrollingIntoTile('[title*="MAF Per Car"]',0)
				robotHands.clickOnAccountingIcon()
				robotHands.verifiyingTheTileWithDefaultFrequency(Cypress.env('MAFCostPerCar'),"merchant_account_fees_cost_per_car")
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="MAF Per Car "]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="MAF Per Car "]','MAF Per Car ')
			})

			it('Verifying the MAF Cost Per Car drill-down view', () => {
				robotHands.clickOnTile('div[title="MAF Per Car "]')
		 ...
		 RoleFilter(['regression'], () => {
			it('Verifying the context of Table', () => {
				...
				robotHands.verifyingQuarterlyFrequency(Cypress.env('MAFCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesYearlyFilterOption()
				robotHands.clickOnYearlyFilterOption()
				robotHands.verifyingYearlyFrequency(Cypress.env('MAFCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesMonthlyFilterOption()
				robotHands.clickOnMonthlyFilterOption()
				robotHands.verifyingMonthlyFrequency(Cypress.env('MAFCostPerCar'))
