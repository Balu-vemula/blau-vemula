	...
	describe('Verifying the functionality of Profit And Loss Comparison KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title="P&L Comparison (Quarterly)"]',0)
				robotEyes.seesTitleOfKPI('[title="P&L Comparison (Quarterly)"]','P&L Comparison (Quarterly)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title="P&L Comparison (Quarterly)"]','P&L Comparison (Quarterly)')
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="P&L Comparison (Quarterly)"]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="P&L Comparison (Quarterly)"]','P&L Comparison (Quarterly)')
			})

			it('Verifying the Profit And Loss Comparison drill-down view', () => {
				robotHands.clickOnTile('div[title="P&L Comparison (Quarterly)"]')
		 ...
		 RoleFilter(['regression'], () => {
			it('Verifying the context of Table', () => {					
				...
				robotHands.clickOnQuarterlyInFirstDropDown()
				robotHands.selectQuarterFromTheQuarterlyInFirstDropDown()
				robotHands.clickOnQuarterlyInSecondDropDown()
				robotHands.selectQuarterFromTheQuarterlyInSecondDropDown()
