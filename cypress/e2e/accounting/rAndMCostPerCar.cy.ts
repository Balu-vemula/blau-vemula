...
context('Verifying the functionality of R&M Cost Per Car KPI', () => {
	describe('Verifying the functionality of R&M Cost Per Car KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title*="R&M Cost Per Car"]',0)
				robotEyes.seesTitleOfKPI('[title*="R&M Cost Per Car"]','R&M Cost Per Car (YTD)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title*="R&M Cost Per Car"]','R&M Cost Per Car (YTD)')
			})

			it('Verifying the graph and the value of the Cost Per Car on the tile view', () => {
				robotHands.scrollingIntoTile('[title*="R&M Cost Per Car"]',0)
				robotHands.clickOnAccountingIcon()
				robotHands.verifiyingTheTileWithDefaultFrequency(Cypress.env('RandMCostPerCar'),"repairs_and_maintenance_cost_per_car")
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="R&M Cost Per Car "]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="R&M Cost Per Car "]','R&M Cost Per Car ')
			})

			it('Verifying the R&M Cost Per Car drill-down view', () => {
				robotHands.clickOnTile('div[title="R&M Cost Per Car "]')
		 ...
		 RoleFilter(['regression'], () => {
			it('Verifying the context of Table', () => {
				...
				robotHands.verifyingQuarterlyFrequency(Cypress.env('RandMCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesYearlyFilterOption()
				robotHands.clickOnYearlyFilterOption()
				robotHands.verifyingYearlyFrequency(Cypress.env('RandMCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesMonthlyFilterOption()
				robotHands.clickOnMonthlyFilterOption()
				robotHands.verifyingMonthlyFrequency(Cypress.env('RandMCostPerCar'))
