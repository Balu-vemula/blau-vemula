...
context('Verifying the functionality of Water Cost Per Car KPI', () => {
	describe('Verifying the functionality of Water Cost Per Car KPI', () => {
		 RoleFilter(['smoke', 'regression'], () => {
			...
			it('Verifying the title of the KPI', () => {
				robotHands.scrollingIntoTile('[title*="Water Cost Per Car"]',0)
				robotEyes.seesTitleOfKPI('[title*="Water Cost Per Car"]','Water Cost Per Car (YTD)')
				robotEyes.verifyingTheTitleWithDefaultDatePickerFrequency('[title*="Water Cost Per Car"]','Water Cost Per Car (YTD)')
			})

			it('Verifying the graph and the value of the Water Cost Per Car on the tile view', () => {
				robotHands.scrollingIntoTile('[title*="Water Cost Per Car',0)
				robotHands.clickOnAccountingIcon()
				robotHands.verifiyingTheTileWithDefaultFrequency(Cypress.env('WaterCostPerCar'),"water_cost_per_car")
			})
			...
			it('Verifying the functionality of the date picker on the tile view', () => {
				...
				robotHands.scrollingIntoTileAfterChangingTheFrequencyFromDefault('[title="Water Cost Per Car "]', 0)
				robotEyes.veryfingTheTitleOfTileViewAfterChangingTheFrequencyFromDefault('[title="Water Cost Per Car "]','Water Cost Per Car ')
			})

			it('Verifying the Water Cost Per Car drill-down view', () => {
				robotHands.clickOnTile('div[title="Water Cost Per Car "]')
		 ...
		 RoleFilter(['regression'], () => {
			it('Verifying the context of Table', () => {
				...
				robotHands.verifyingQuarterlyFrequency(Cypress.env('WaterCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesYearlyFilterOption()
				robotHands.clickOnYearlyFilterOption()
				robotHands.verifyingYearlyFrequency(Cypress.env('WaterCostPerCar'))
				robotHands.clickOnFrequencyFilter()
				robotEyes.seesMonthlyFilterOption()
				robotHands.clickOnMonthlyFilterOption()
				robotHands.verifyingMonthlyFrequency(Cypress.env('WaterCostPerCar'))
