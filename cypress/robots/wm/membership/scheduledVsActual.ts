...
export class RobotEyes extends BaseEyes {
    ...
    seesTheTitleOfKPI() {
        this.seesDomVisible('[title*="Scheduled vs. Actual (YTD)"]')
    }
    verifiesTheTitleOfKPIWithDefaultFrequency(title:string) {
		this.wait(3000)
        this.seesDomContainText('[title*="Scheduled vs. Actual (YTD)"]',title)
    }
	...
	seesDatePickerIsVisibleOnDrillDownView() {
		this.wait(3000)
		this.seesDomVisible('button[aria-describedby="SCHEDULED_VS_ACTUAL"]')
	}
	...
	seesGraphOnDrillDownView() {
		this.wait(3000)
		this.seesIdVisible('drill-down-SCHEDULED_VS_ACTUAL')
	...
	seesTheSortButtonsForScheduledAccounts() {
	...
	seesTheSortButtonsForScheduledBilling() {
	...
	verifyTheCancelButtonInTheLocationInDrillDownView() {
	...
	verifyTheApplyButtonInTheLocationInDrillDownView() {
		this.seesDomVisible(
	...
	verifyTheDatePickerFrequenciesInDrillDownView() {
	...
	verifyTheCancelButtonInTheDatePickerInDrillDownView() {
		this.seesDomVisible(
	...
	verifyTheApplyButtonInTheDatePickerInDrillDownView() {
		this.seesDomVisible(
...
export class RobotHands extends BaseHands {
    ...
    scrollingToSeeTitleOfKPI() {
        this.wait(10000)
        this.scrollIntoElement('[title*="Scheduled vs. Actual (YTD)"]',0)
    }
	...
	clickOnDatePickerInDrillDownView() {
		this.wait(3000)
		this.clickOnDomElement('[aria-describedby="SCHEDULED_VS_ACTUAL"]')
	}
	...
	ScrollToSeePercentageRecievedHeaderInTable() {
		this.scrollIntoElementWithOutIndex('th:nth-child(5) span:nth-child(1) span:nth-child(1)')
	}
	...
	clicksOnAscendingOrderScheduledAccounts() {
		this.clickOnDomElementWithIndex(
			'[class*="line-flex flex-col gap"] svg path',
			3
		)
	}
	clicksOnDescendingOrderScheduledAccounts() {
		this.clickOnDomElementWithIndex(
			'[class*="line-flex flex-col gap"] svg path',
			2
		)
	}
	clicksOnAscendingOrderScheduledBilling() {
		this.clickOnDomElementWithIndex(
			'[class*="line-flex flex-col gap"] svg path',
			5
		)
	}
	clicksOnDescendingOrderScheduledBilling() {
		this.clickOnDomElementWithIndex(
			'[class*="line-flex flex-col gap"] svg path',
			4
		)
	}
	clicksOnAscendingOrderActualBilledAccount() {
		this.clickOnDomElementWithIndex(
			'[class*="line-flex flex-col gap"] svg path',
			7
		)
	}
	clicksOnDescendingOrderActualBilledAccount() {
		this.clickOnDomElementWithIndex(
			'[class*="line-flex flex-col gap"] svg path',
			6
		)
